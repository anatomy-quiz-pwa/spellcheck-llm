...
def main():
    ...
    with st.sidebar:
        ...
        # 處理選項
        st.subheader("🔧 處理選項")
        extract_text = st.checkbox("提取文字", value=True)
        analyze_terms = st.checkbox("分析術語", value=True)
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            if termbase_file is not None:
                ...
                # 簡單的術語檢測
                if analyze_terms and not termbase.empty:
                    detected_terms = []
                    for index, row in termbase.iterrows():
                        zh_term = str(row.get('zh', ''))
                        en_term = str(row.get('en', ''))
                        
                        if zh_term in full_text or en_term in full_text:
                            detected_terms.append({
                                "術語": f"{zh_term} ({en_term})",
                                "狀態": "已檢測到"
                            })
                    
                    analysis_results["檢測到的術語"] = len(detected_terms)
                    
                    if detected_terms:
                        st.subheader("📋 檢測到的術語")
                        detected_df = pd.DataFrame(detected_terms)
                ...
                with col4:
                    st.metric("📚 詞庫條目", analysis_results["詞庫條目數"])
